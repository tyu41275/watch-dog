...
function classifyLine(file, line, report) {
  for (const { family, regex } of endpointPatterns) {
    ...
    const endpoint = { file, family, port: Number(match[1]), address: match[2],
      syscall: line.includes("connect(") ? "connect" : /send(?:to|msg|mmsg)\(/u.test(line) ?
        "datagram" : "other", success: resultOk(line) };
...
export async function auditTrace({ traceDirectory, censusPath, summaryPath,
  ...
  const report = { trace_files: files.length, census_pids: [], spawned_pids: [],
    trace_root_pids: [], unreachable_trace_pids: [], ancestry_edges: [], endpoints: [],
  ...
  const spawnPattern = /(?:clone3?|fork|vfork)\([^\n]*\)\s+=\s+([1-9][0-9]*)$/gmu;
  for (const [file, body] of bodies) {
    const parent = file.split(".").at(-1);
    for (const match of body.matchAll(spawnPattern)) report.ancestry_edges.push([parent, match[1]]);
    if (/execve\("\/usr\/bin\/node".*"e2e\/runtime-supervisor\.mjs"/u.test(body)) {
      report.trace_root_pids.push(parent);
    }
  }
  report.spawned_pids = [...new Set(report.ancestry_edges.map(([, child]) => child))].sort();
  const reachable = new Set(report.trace_root_pids); let changed = true;
  while (changed) {
    changed = false;
    for (const [parent, child] of report.ancestry_edges) {
      if (reachable.has(parent) && !reachable.has(child)) { reachable.add(child); changed = true; }
    }
  }
  report.missing_trace_pids = [...new Set([
    ...report.census_pids.filter((pid) => pid !== "1" && !ignored.has(pid) && !traced.has(pid)),
    ...report.spawned_pids.filter((pid) => !traced.has(pid)),
  ])].sort();
  report.unreachable_trace_pids = [...traced].filter((pid) => !reachable.has(pid)).sort();
  ...
  if (report.successful_non_loopback.length || report.successful_dns.length ||
    report.trace_root_pids.length !== 1 || report.unreachable_trace_pids.length ||
