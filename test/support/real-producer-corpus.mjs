...
function operationStore(core, events, operation) {
  return async (submitted) => {
    const scanId = core.putLiveResult(SESSION, submitted, Date.parse(NOW));
...
function commitPaste(core, events, operation, prefix) {
  const receipt = core.commitPaste(SESSION, prefix.input, prefix.journal, Date.parse(NOW));
  for (const scanId of receipt.scan_ids) {
    const stored = core.getResult(SESSION, scanId, Date.parse(NOW)).result;
    events.push({ operation, receipt_id: receipt.receipt_id, scan_id: scanId, stored: clone(stored) });
  }
  return receipt;
}
...
async function captureRealProducerCorpusExclusive() {
  ...
  try {
    ...
    const pasteUrl = commitPaste(core, events.stores, "paste_url", await executePasteScan(pasteUrlRequest, { now: () => new Date(NOW), provider: recordingProvider(matchAdapter, events.provider_requests, "paste_url"), fetch_seams: { resolver, fetcher: successFetcher } }));
    ...
    const pasteHtml = commitPaste(core, events.stores, "paste_html", await executePasteScan(pasteHtmlRequest, { now: () => new Date(NOW), provider: recordingProvider(matchAdapter, events.provider_requests, "paste_html") }));
    ...
    const redirectLoop = commitPaste(core, events.stores, "redirect_loop", await executePasteScan(loopRequest, { now: () => new Date(NOW), fetch_seams: { resolver, fetcher: loopFetcher } }));
    ...
    const paste256 = commitPaste(core, events.stores, "paste_256", await executePasteScan(paste256Request, { now: () => new Date(NOW) }));
    ...
    const pairs = events.stores.map(({ operation, receipt_id = null, scan_id, stored }) => ({ operation, receipt_id: receipt_id ?? scan_id, result_id: stored.scan_id }));
